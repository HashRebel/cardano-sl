...
deriving instance
    ( Show (GenericBlockHeader bodyProof consensus extraH)
    , Show body
    , Show extraB
    ) => Show (GenericBlock bodyProof consensus extraH body extraB)

-- Can we derive this in brackets?
deriving instance
    ( Eq bodyProof
    , Eq consensus
    , Eq extraH
    , Eq body
    , Eq extraB
    ) => Eq (GenericBlock bodyProof consensus extraH body extraB)
